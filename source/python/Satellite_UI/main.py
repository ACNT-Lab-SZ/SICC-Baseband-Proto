        ...
        while True:
            try:
                ...
                if start < 0:
                    continue
                state, _ = decoder.raw_decode(text[start:])
                if state.get("type") in ("ntn_channel_subplots", "ntn_channel_state", "ntn_channel_vis"):
...
class PyqtgraphDdSurfaceWidget(QWidget):
    def __init__(self, parent=None):
        ...
        legend = QLabel(
            ...
            "&nbsp;&nbsp; <span style='color:#FBBF24;font-family:Consolas;font-weight:bold;'>Z: Normalized intensity</span>"
        )
...
class MatplotlibDdSurfaceWidget(QWidget):
    ...
    def style_axes(self, rows, cols):
        ...
        self.ax.set_zlabel("Normalized intensity", color="#FBBF24", labelpad=6)
    ...
    def draw_surface(self, grid, force=False):
        ...
        if arr.ndim != 2 or not arr.size:
            return
        arr = np.nan_to_num(arr, nan=0.0, posinf=1.0, neginf=0.0)
        lo, hi = np.percentile(arr, [3, 99.7])
        if hi > lo:
            arr = (arr - lo) / (hi - lo)
...
class ExternalDdSurfaceWidget(QWidget):
    ...
    def update_scaled_pixmap(self):
        ...
        painter.drawText(legend_rect.adjusted(10, 40, -10, -8), Qt.AlignRight | Qt.AlignVCenter, "Z Normalized intensity")
...
class WebDdSurfaceWidget(QWidget):
    ...
    def send_grid(self, arr, force=False):
        ...
        if not self.page_ready:
            return
        payload = json.dumps({"grid": arr.tolist()}, ensure_ascii=False)
...
class SoftwareDdSurfaceWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(12, 42, "Z: Normalized intensity")
...
class SatelliteSystemUI(QMainWindow):
    ...
    def init_dashboard_panel(self, layout):
        ...
        self.path_gain_plot = pg.PlotWidget(
            title="<span style='color: #FBBF24; font-size: 10pt; font-family: Microsoft YaHei;'>Current Path Magnitudes</span>")
        style_plot(self.path_gain_plot)
        self.path_gain_plot.setLabel('bottom', 'Path index', **{'font-family': 'Consolas'})
        self.path_gain_plot.setLabel('left', '|h|', **{'font-family': 'Consolas'})
        self.path_clean_bar = None
        self.path_noisy_bar = None

        self.sample_window_plot = pg.PlotWidget(
            title="<span style='color: #F472B6; font-size: 10pt; font-family: Microsoft YaHei;'>Training Sample Construction</span>")
        style_plot(self.sample_window_plot)
        self.sample_window_plot.setLabel('bottom', 'Time frame', **{'font-family': 'Consolas'})
        self.sample_window_plot.setYRange(0, 1)
        self.sample_window_plot.getAxis('left').setTicks([])
        self.sample_progress_curve = self.sample_window_plot.plot(
            pen=pg.mkPen(color=(170, 170, 170, 180), width=12))
        self.sample_input_curve = self.sample_window_plot.plot(
            pen=pg.mkPen(color='#2D7DD2', width=16))
        self.sample_target_curve = self.sample_window_plot.plot(
            pen=pg.mkPen(color='#F59E0B', width=16))
        self.sample_current_line = pg.InfiniteLine(
            angle=90, movable=False, pen=pg.mkPen(color='#EF4444', width=2))
        self.sample_window_plot.addItem(self.sample_current_line)
        self.sample_status_text = pg.TextItem(color='#E5E7EB', anchor=(0, 0))
        self.sample_window_plot.addItem(self.sample_status_text)

        csi_subplots_layout.addWidget(self.dd_widget, stretch=1)
        csi_subplots_layout.addWidget(self.path_gain_plot, stretch=1)
        csi_subplots_layout.addWidget(self.sample_window_plot, stretch=1)
    ...
    def probe_constellation_page(self):
        ...
        """
        self.browser.page().runJavaScript(script, self.on_constellation_probe_result)
    ...
    def update_channel_state_view(self, state):
        ...
        if state.get("type") != "ntn_channel_subplots":
            return
        ...
        clean_mag = as_float_array(state.get("cleanPathMagnitude"))
        noisy_mag = as_float_array(state.get("noisyPathMagnitude"))
        n_paths = min(clean_mag.size, noisy_mag.size)
        if n_paths:
            clean_mag = clean_mag[:n_paths]
            noisy_mag = noisy_mag[:n_paths]
            x = np.arange(1, n_paths + 1, dtype=float)
            for item_name in ("path_clean_bar", "path_noisy_bar"):
                item = getattr(self, item_name, None)
                if item is not None:
                    self.path_gain_plot.removeItem(item)
            self.path_clean_bar = pg.BarGraphItem(
                x=x - 0.18, height=clean_mag, width=0.34,
                brush=pg.mkBrush(QColor(37, 99, 235, 220)))
            self.path_noisy_bar = pg.BarGraphItem(
                x=x + 0.18, height=noisy_mag, width=0.34,
                brush=pg.mkBrush(QColor(249, 115, 22, 220)))
            self.path_gain_plot.addItem(self.path_clean_bar)
            self.path_gain_plot.addItem(self.path_noisy_bar)
            self.path_gain_plot.setXRange(0.4, n_paths + 0.6, padding=0.02)
            self.path_gain_plot.setYRange(0.0, max(0.1, float(max(np.max(clean_mag), np.max(noisy_mag))) * 1.15), padding=0.0)

        frame = int(state.get("frame", 1) or 1)
        total_frames = int(state.get("totalFrames", max(1000, frame)) or max(1000, frame))
        sample_count = 0
        has_sample_state = any(
            key in state
            for key in ("inputWindow", "targetWindow", "stride", "sampleCount")
        )
        if has_sample_state:
            input_window = int(state.get("inputWindow", 96) or 96)
            target_window = int(state.get("targetWindow", 24) or 24)
            stride = int(state.get("stride", 12) or 12)
            sample_count = int(state.get("sampleCount", 0) or 0)

            self.sample_window_plot.setXRange(1, total_frames, padding=0.01)
            self.sample_progress_curve.setData([1, frame], [0.50, 0.50])
            self.sample_current_line.setPos(frame)
            if sample_count > 0:
                x_start = 1 + (sample_count - 1) * stride
                x_end = x_start + input_window - 1
                y_start = x_end + 1
                y_end = y_start + target_window - 1
                self.sample_input_curve.setData([x_start, x_end], [0.38, 0.38])
                self.sample_target_curve.setData([y_start, y_end], [0.66, 0.66])
                status = f"Generated samples: {sample_count}\nX: {x_start}-{x_end}    Y: {y_start}-{y_end}"
            else:
                self.sample_input_curve.setData([], [])
                self.sample_target_curve.setData([], [])
                need = input_window + target_window
                status = f"Collecting CSI: {frame} / {need} frames\nWaiting for enough frames"
            self.sample_status_text.setText(status)
            self.sample_status_text.setPos(max(1, total_frames * 0.03), 0.86)
        ...
        if throughput_value is not None:
            try:
                ...
                if np.isfinite(throughput_value):
                    ...
                    self.throughput_plot.setXRange(1, max(1200, frame), padding=0.02)
        ...
        if mcs_value is not None:
            try:
                ...
                if np.isfinite(mcs_value):
                    ...
                    self.amc_state_plot.setXRange(1, max(1200, frame), padding=0.02)
