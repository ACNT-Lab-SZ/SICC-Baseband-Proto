...
static Options parse_options(int argc, char** argv)
{
    ...
    if ((opt.gpu_pipeline || opt.gpu_tx_baseband || opt.gpu_rx_sync || opt.gpu_rx_demod) &&
        !gpu_mode_ok) {
        throw std::runtime_error("GPU full-pipeline stages are wired into --mode gpu-sim, trx, trx-capture, and replay");
    }
    return opt;
}
...
static int run_gpu_sim(const Options& opt, const PhyConfig& phy, const LdpcCode& code, const fec::RxDecoderPlan& rx_plan)
{
    ...
    if (std::isfinite(opt.sim_snr_db)) {
        std::cout << "[GPU-SIM] AWGN injection is host-side; TX baseband, RX sync/demod/LLR and CUDA decoder remain on GPU.\n";
    }

    GpuTxBasebandChain gpu_tx(phy, code, &precomp_channel, &actual_channel);
    GpuRxSyncExtractor gpu_sync(phy, code);
    GpuRxDemodChain gpu_demod(phy, code, &precomp_channel);

    const int frames = opt.frames > 0
        ? opt.frames
        : (file_mode ? static_cast<int>(tx_media->total_chunks) : 4);
    std::vector<cf32> rxbuf;
    rxbuf.reserve(static_cast<size_t>(std::min(std::max(frames, 1), 8)) * static_cast<size_t>(phy.frame_len()));
    size_t rxbase = 0;
    SyncState sync_state = SyncState::Acquisition;
    SyncStats sync_stats;
    DecodeProfile decode_profile;
    ...
    uint64_t sent_frames = 0;
    uint32_t expected_test_frame_id = 0;
    std::mt19937 noise_rng(static_cast<uint32_t>(opt.test_seed) ^ 0x679aa031u);
    std::normal_distribution<float> noise_dist(0.0f, 1.0f);

    auto add_awgn_if_requested = [&](std::vector<cf32>& samples) {
        if (!std::isfinite(opt.sim_snr_db) || samples.empty()) {
            return;
        }
        const double frequency_signal_power = std::max(active_subcarrier_power(samples, phy), 1e-12);
    ...
    for (int i = 0; i < frames; ++i) {
        ...
        if (file_mode) {
            const auto info = tx_media->build_info_bits(static_cast<uint32_t>(i));
            tx = gpu_tx.build_media_frame(info);
        } else {
            tx = gpu_tx.build_test_frame(static_cast<uint32_t>(i));
        }
        add_awgn_if_requested(tx);
        rxbuf.insert(rxbuf.end(), tx.begin(), tx.end());
        ++sent_frames;

        while (true) {
            ExtractedFrame extracted;
            if (!gpu_sync.extract(rxbuf, rxbase, extracted, sync_state, sync_stats)) {
                break;
            }

            FrameDecodeResult r = gpu_demod.process(
                extracted,
                cuda_bp_decoder.get(),
                cuda_decoder.get(),
                opt.ldpc_max_iter,
                ...
                opt.profile_pipeline ? &decode_profile : nullptr);
            if (!file_mode) {
                const auto ref_info = build_info_bits(phy, code, expected_test_frame_id);
                int bit_errors = 0;
                for (size_t b = 0; b < std::min(r.info_bits.size(), ref_info.size()); ++b) {
                    bit_errors += (r.info_bits[b] != ref_info[b]) ? 1 : 0;
                }
                r.frame_id = expected_test_frame_id;
