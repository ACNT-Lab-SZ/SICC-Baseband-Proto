...
__global__ void copy_systematic_info_kernel(
    const uint8_t* __restrict__ info_bits,
    uint8_t* __restrict__ coded_bits,
    int blocks,
    int k,
...
{
    const int bit = blockIdx.x * blockDim.x + threadIdx.x;
    const int total = blocks * k;
    if (bit >= total) {
        return;
    }
    const int cw = bit / k;
    const int i = bit - cw * k;
    coded_bits[cw * n + i] = static_cast<uint8_t>(info_bits[bit] & 1u);
}

__global__ void ldpc_encode_dvb_s2_accumulator_kernel(
    const uint8_t* __restrict__ row_sums,
    uint8_t* __restrict__ coded_bits,
    int blocks,
    int k,
    int m,
    int n)
{
    const int cw = blockIdx.x * blockDim.x + threadIdx.x;
    if (cw >= blocks) {
        return;
    }
    uint8_t previous_parity = 0;
    for (int r = 0; r < m; ++r) {
        const uint8_t row_sum = row_sums[cw * m + r];
        const uint8_t parity = static_cast<uint8_t>(row_sum ^ ((r == 0) ? 0u : previous_parity));
        coded_bits[cw * n + k + r] = parity;
        previous_parity = parity;
    }
}

__global__ void compute_dvb_s2_row_sums_kernel(
    const uint8_t* __restrict__ info_bits,
    const int* __restrict__ row_info_offsets,
    const int* __restrict__ row_info_cols,
    uint8_t* __restrict__ row_sums,
    int blocks,
    int k,
    int m)
{
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    const int total = blocks * m;
    if (idx >= total) {
        return;
    }
    const int cw = idx / m;
    const int r = idx - cw * m;
    const uint8_t* info = info_bits + cw * k;
    uint8_t sum = 0;
    const int begin = row_info_offsets[r];
    const int end = row_info_offsets[r + 1];
    for (int p = begin; p < end; ++p) {
        sum ^= static_cast<uint8_t>(info[row_info_cols[p]] & 1u);
    }
    row_sums[idx] = sum;
}
...
__device__ __forceinline__ float uniform_01(uint32_t seed, uint32_t idx)
{
    const uint32_t h = hash_u32(seed ^ (idx * 0x9e3779b9u));
    return (static_cast<float>(h) + 1.0f) * 2.3283064365386963e-10f;
}

__device__ __forceinline__ float normal01(uint32_t seed, uint32_t idx)
{
    const float u1 = fmaxf(uniform_01(seed, idx * 2u), 1.0e-7f);
    const float u2 = uniform_01(seed, idx * 2u + 1u);
    return sqrtf(-2.0f * logf(u1)) * cosf(2.0f * kPiF * u2);
}

__global__ void apply_actual_channel_and_frequency_awgn_kernel(
    cuFloatComplex* __restrict__ grid,
    const cuFloatComplex* __restrict__ actual_h,
    const int* __restrict__ used_indices,
    const int* __restrict__ data_symbol_index,
    int nfft,
    int num_symbols,
    ...
    uint32_t noise_seed)
{
    ...
    const int sc = idx - sym * active_sc;
    const int fft_bin = shifted_to_natural_bin_dev(used_indices[sc], nfft);
    const int grid_idx = sym * nfft + fft_bin;
    cuFloatComplex value = grid[grid_idx];
    if (actual_h != nullptr) {
        value = cmul(value, actual_h[idx]);
    }
    if (frequency_noise_var > 0.0f && data_symbol_index[sym] >= 0) {
        const float sigma = sqrtf(fmaxf(frequency_noise_var, 0.0f) * 0.5f);
        value = cadd(value, make_cuFloatComplex(
            sigma * normal01(noise_seed, static_cast<uint32_t>(idx)),
            sigma * normal01(noise_seed ^ 0xa5a5a5a5u, static_cast<uint32_t>(idx))));
    }
    grid[grid_idx] = value;
}
...
struct DeviceSyncResult {
    ...
    float cfo_hz;
    int fallback_start;
    float fallback_metric;
};

__global__ void earliest_metric_kernel(
    ...
    float threshold)
{
    ...
    __shared__ float max_metric[kSyncThreads];
    __shared__ int max_index[kSyncThreads];

    int local_index = candidates;
    float local_max_metric = -1.0f;
    int local_max_index = 0;
    for (int i = threadIdx.x; i < candidates; i += blockDim.x) {
        const float m = metric[i];
        if (m >= threshold && i < local_index) {
            local_index = i;
        }
        if (m > local_max_metric) {
            local_max_metric = m;
            local_max_index = i;
        }
    }
    ...
    max_metric[threadIdx.x] = local_max_metric;
    max_index[threadIdx.x] = local_max_index;
    __syncthreads();

    for (int stride = blockDim.x / 2; stride > 0; stride >>= 1) {
        if (threadIdx.x < stride && best_index[threadIdx.x + stride] < best_index[threadIdx.x]) {
            best_index[threadIdx.x] = best_index[threadIdx.x + stride];
        }
        if (threadIdx.x < stride && max_metric[threadIdx.x + stride] > max_metric[threadIdx.x]) {
            max_metric[threadIdx.x] = max_metric[threadIdx.x + stride];
            max_index[threadIdx.x] = max_index[threadIdx.x + stride];
        }
        __syncthreads();
    }

    if (threadIdx.x == 0) {
        result->fallback_start = max_index[0];
        result->fallback_metric = max_metric[0];
        if (best_index[0] >= candidates) {
            result->start = -1;
            result->metric = 0.0f;
            result->cfo_hz = 0.0f;
        } else {
            result->start = best_index[0];
            result->metric = metric[best_index[0]];
            result->cfo_hz = 0.0f;
        }
    }
}

__global__ void refine_preamble_metric_kernel(
    const cuFloatComplex* __restrict__ samples,
    const cuFloatComplex* __restrict__ preamble,
    DeviceSyncResult* __restrict__ result,
    int candidates,
    ...
    float sample_rate)
{
    __shared__ float s_re[kSyncThreads];
    __shared__ float s_im[kSyncThreads];
    __shared__ float s_eng[kSyncThreads];
    __shared__ int best_start;
    __shared__ float best_metric;
    ...
    for (int i = threadIdx.x; i < preamble_len; i += blockDim.x) {
        pre_energy += cnorm(preamble[i]);
    }
    s_eng[threadIdx.x] = pre_energy;
    __syncthreads();
    for (int stride = blockDim.x / 2; stride > 0; stride >>= 1) {
        if (threadIdx.x < stride) {
            s_eng[threadIdx.x] += s_eng[threadIdx.x + stride];
        }
        __syncthreads();
    }
    const float total_pre_energy = fmaxf(s_eng[0], 1.0e-20f);

    for (int pass = 0; pass < 2; ++pass) {
        const int center = (pass == 0) ? result->start : result->fallback_start;
        if (center < 0 || center >= candidates) {
            continue;
        }
        if (pass == 1 && best_metric >= threshold) {
            continue;
        }

        const int lo = max(0, center - refine_radius);
        const int hi = min(candidates - 1, center + refine_radius);
        for (int d = lo; d <= hi; ++d) {
            float re = 0.0f;
            float im = 0.0f;
            float eng = 0.0f;
            for (int i = threadIdx.x; i < preamble_len; i += blockDim.x) {
                const cuFloatComplex y = samples[d + i];
                const cuFloatComplex p = preamble[i];
                const cuFloatComplex c = cmul(y, cconj(p));
                re += c.x;
                im += c.y;
                eng += cnorm(y);
            }
            ...
            if (threadIdx.x == 0) {
                const float m = (s_re[0] * s_re[0] + s_im[0] * s_im[0]) /
                                fmaxf(s_eng[0] * total_pre_energy, 1.0e-20f);
                if (m > best_metric) {
                    best_metric = m;
                    best_start = d;
                }
    ...
    if (threadIdx.x == 0) {
        if (best_start < 0 || best_metric < threshold) {
            result->start = -1;
        ...
        cuFloatComplex cfo_acc = make_cuFloatComplex(0.0f, 0.0f);
        for (int i = 0; i < half_len; ++i) {
            cfo_acc = cadd(cfo_acc, cmul(cconj(samples[best_start + i]), samples[best_start + i + half_len]));
        }
        const float phase = atan2f(cfo_acc.y, cfo_acc.x);
    ...
    bool use_pilot_channel_est)
{
    ...
    cuFloatComplex h = make_cuFloatComplex(1.0f, 0.0f);
    if (use_pilot_channel_est) {
        const cuFloatComplex h0 = pilot_h[p0 * active_sc + sc];
        const cuFloatComplex h1 = pilot_h[p1 * active_sc + sc];
        h = cadd(cscale(h0, 1.0f - alpha), cscale(h1, alpha));
    }
...
struct GpuStaticTables {
    ...
    std::vector<cuFloatComplex> actual_h;   // optional [num_symbols][active_sc]
};
...
public:
    ...
    void upload_static_tables(const GpuStaticTables& h)
    {
        ...
        use_dvb_s2_accumulator_encoder_ = h.dvb_s2_accumulator;
        if (h.dvb_s2_accumulator) {
            require_size(h.row_info_offsets.size(), static_cast<size_t>(cfg_.code_n - cfg_.code_k + 1), "row_info_offsets");
            d_row_info_offsets_.allocate(h.row_info_offsets.size());
            copy_to_device(d_row_info_offsets_, h.row_info_offsets);
            if (!h.row_info_cols.empty()) {
                d_row_info_cols_.allocate(h.row_info_cols.size());
                copy_to_device(d_row_info_cols_, h.row_info_cols);
            }
        }
        ...
        if (!h.actual_h.empty()) {
            require_size(h.actual_h.size(), static_cast<size_t>(cfg_.num_symbols * cfg_.active_sc), "actual_h");
            d_actual_h_.allocate(h.actual_h.size());
            copy_to_device(d_actual_h_, h.actual_h);
        }
    }

    const cuFloatComplex* submit_tx_frame_device_async(
        int slot,
        ...
        bool add_frequency_noise)
    {
        ...
        if (!use_dvb_s2_accumulator_encoder_ && d_generator_.get() == nullptr) {
        ...
        if (use_dvb_s2_accumulator_encoder_) {
            const int info_threads = 256;
            copy_systematic_info_kernel<<<div_up(cfg_.blocks_per_frame * cfg_.code_k, info_threads),
                                          info_threads,
                                          0,
                                          tx_encode_stream_>>>(
                s.info_bits.get(),
                ...
                cfg_.code_k,
                cfg_.code_n);
            check_cuda(cudaGetLastError(), "copy_systematic_info_kernel");

            const int row_threads = 256;
            compute_dvb_s2_row_sums_kernel<<<div_up(cfg_.blocks_per_frame * (cfg_.code_n - cfg_.code_k), row_threads),
                                             row_threads,
                                             0,
                                             tx_encode_stream_>>>(
                s.info_bits.get(),
                d_row_info_offsets_.get(),
                d_row_info_cols_.get(),
                s.row_sums.get(),
                cfg_.blocks_per_frame,
                cfg_.code_k,
                cfg_.code_n - cfg_.code_k);
            check_cuda(cudaGetLastError(), "compute_dvb_s2_row_sums_kernel");

            const int acc_threads = 128;
            ldpc_encode_dvb_s2_accumulator_kernel<<<div_up(cfg_.blocks_per_frame, acc_threads),
                                                    acc_threads,
                                                    0,
                                                    tx_encode_stream_>>>(
                s.row_sums.get(),
                ...
                cfg_.code_n - cfg_.code_k,
                cfg_.code_n);
            check_cuda(cudaGetLastError(), "ldpc_encode_dvb_s2_accumulator_kernel");
        } else {
        ...
        if (d_actual_h_.get() != nullptr || (add_frequency_noise && frequency_noise_var > 0.0f)) {
            apply_actual_channel_and_frequency_awgn_kernel<<<div_up(re_total, re_threads), re_threads, 0, tx_ofdm_stream_>>>(
                s.tx_grid.get(),
                d_actual_h_.get(),
                d_used_indices_.get(),
                d_data_symbol_index_.get(),
                ...
                cfg_.num_symbols,
                cfg_.active_sc,
                add_frequency_noise ? frequency_noise_var : 0.0f,
                static_cast<uint32_t>(noise_seed ^ (noise_seed >> 32)));
            check_cuda(cudaGetLastError(), "apply_actual_channel_and_frequency_awgn_kernel");
        }
    ...
    {
        Slot& s = checked_slot(slot);
        const cuFloatComplex* d_frame = submit_tx_frame_device_async(
            slot,
            h_info_bits,
            0.0f,
            0,
    ...
    void run_rx_sync_on_device(int slot, const cuFloatComplex* d_rx_samples, int sample_count, GpuSyncResult* h_result)
    {
        Slot& s = checked_slot(slot);
        ...
        earliest_metric_kernel<<<1, kSyncThreads, 0, rx_sync_stream_>>>(
            s.sync_metric.get(),
            s.sync_result.get(),
            candidates,
            cfg_.sync_threshold);
        check_cuda(cudaGetLastError(), "earliest_metric_kernel");

        refine_preamble_metric_kernel<<<1, kSyncThreads, 0, rx_sync_stream_>>>(
            d_rx_samples,
            d_preamble_.get(),
            s.sync_result.get(),
            candidates,
            ...
            cfg_.sync_threshold,
            cfg_.sample_rate);
        check_cuda(cudaGetLastError(), "refine_preamble_metric_kernel");
    ...
    void submit_rx_sync_async(int slot, const cuFloatComplex* h_rx_samples, int sample_count, GpuSyncResult* h_result)
    {
        ...
        check_cuda(cudaMemcpyAsync(
                       ...
                       rx_sync_stream_),
                   "copy RX samples");
        run_rx_sync_on_device(slot, s.rx_samples.get(), sample_count, h_result);
    }

    // RX sync stage: device samples -> GPU Schmidl-Cox search. This avoids the
    // host bridge copy used by submit_rx_sync_async().
    void submit_rx_sync_device_async(int slot, const cuFloatComplex* d_rx_samples, int sample_count, GpuSyncResult* h_result)
    {
        if (d_rx_samples == nullptr) {
            throw std::runtime_error("RX device sample pointer is null");
        }
        run_rx_sync_on_device(slot, d_rx_samples, sample_count, h_result);
    }
    ...
    const float* submit_rx_demod_llr_async(int slot, const cuFloatComplex* d_samples_override = nullptr)
    {
        Slot& s = checked_slot(slot);
        ...
        const cuFloatComplex* d_samples = d_samples_override != nullptr ? d_samples_override : s.rx_samples.get();
    ...
    const float* submit_rx_demod_llr_device_frame_async(int slot, const cuFloatComplex* d_frame, const GpuSyncResult& sync)
    {
        if (d_frame == nullptr) {
            throw std::runtime_error("RX device frame pointer is null");
        }
        Slot& s = checked_slot(slot);
        if (cfg_.frame_samples() > s.max_rx_samples) {
            throw std::runtime_error("RX frame exceeds slot allocation");
        }

        DeviceSyncResult d_sync{};
        d_sync.start = 0;
        d_sync.metric = sync.metric;
        d_sync.cfo_hz = sync.cfo_hz;
        d_sync.fallback_start = 0;
        d_sync.fallback_metric = sync.metric;
        check_cuda(cudaMemcpyAsync(
                       s.sync_result.get(),
                       &d_sync,
                       sizeof(DeviceSyncResult),
                       cudaMemcpyHostToDevice,
                       rx_sync_stream_),
                   "copy device-frame sync result");
        check_cuda(cudaEventRecord(s.sync_done, rx_sync_stream_), "record device-frame sync_done");
        return submit_rx_demod_llr_async(slot, d_frame);
    }
...
private:
    ...
    void allocate_slot(Slot& s)
    {
        ...
        s.row_sums.allocate(static_cast<size_t>(cfg_.blocks_per_frame * (cfg_.code_n - cfg_.code_k)));
    ...
    DeviceBuffer<cuFloatComplex> d_actual_h_;
    DeviceBuffer<int> d_row_info_offsets_;
    DeviceBuffer<int> d_row_info_cols_;
    bool use_dvb_s2_accumulator_encoder_ = false;
...
usrp_gpu_pipeline::GpuPipelineConfig to_device_config(const Config& cfg)
{
    ...
    out.sync_threshold = cfg.sync_threshold;
    out.tx_amplitude = cfg.tx_amplitude;
    out.demod_noise_var = cfg.demod_noise_var;
    out.use_pilot_channel_est = cfg.use_pilot_channel_est;
    return out;
}
...
usrp_gpu_pipeline::GpuStaticTables to_device_tables(const StaticTables& tables)
{
    ...
    out.dvb_s2_accumulator = tables.dvb_s2_accumulator;
    out.row_info_offsets = tables.row_info_offsets;
    out.row_info_cols = tables.row_info_cols;
    ...
    out.actual_h = to_cu_complex_vector(tables.actual_h);
    return out;
}
...
const Complex32* Pipeline::submit_tx_frame_device_async(
    int slot,
    const uint8_t* h_info_bits,
    float frequency_noise_var,
    uint64_t noise_seed,
    bool add_frequency_noise)
{
    return reinterpret_cast<const Complex32*>(impl_->device.submit_tx_frame_device_async(
        slot,
        h_info_bits,
        frequency_noise_var,
        noise_seed,
...
SyncResult Pipeline::find_frame_device(int slot, const Complex32* d_samples, int sample_count)
{
    usrp_gpu_pipeline::GpuSyncResult result;
    impl_->device.submit_rx_sync_device_async(
        slot,
        reinterpret_cast<const cuFloatComplex*>(d_samples),
        sample_count,
        &result);
    impl_->device.synchronize_rx_sync(slot);
    return to_bridge_sync(result);
...
const float* Pipeline::demod_frame_device_to_device_llr(int slot, const Complex32* d_frame, const SyncResult& sync_result)
{
    usrp_gpu_pipeline::GpuSyncResult result;
    result.start = 0;
    result.metric = sync_result.metric;
    result.cfo_hz = sync_result.cfo_hz;
    const float* d_llr = impl_->device.submit_rx_demod_llr_device_frame_async(
        slot,
        reinterpret_cast<const cuFloatComplex*>(d_frame),
        result);
    check_cuda_bridge(cudaStreamSynchronize(impl_->device.rx_demod_stream()), "synchronize RX device-frame demod LLR");
    return d_llr;
}
