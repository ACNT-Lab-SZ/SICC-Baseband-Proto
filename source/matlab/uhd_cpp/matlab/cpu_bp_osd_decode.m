function [infoBits, codeBits, diag] = cpu_bp_osd_decode(llr, H, opts)
...
%
%   The decoder first runs normalized min-sum BP. If BP does not satisfy the
%   parity checks, it falls back to order-N OSD on the BP soft output.
%
%   This file intentionally does not load or call the CUDA DLL. It is a
%   small CPU reference implementation for validating the 128/64 GPU path.
...
for frame = 1:numFrames
    ...
    if ~bp.parityOk
        usedOsd = true;
        osd = osd_decode(bp.bestAppLlr, G, H, opts);
