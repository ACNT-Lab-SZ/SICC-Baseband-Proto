function  [operations, wi] = OperationNum_GRAND_valid_partial_Hamming_Weights(Wi,J_iminus1,I_i,I_iminus1,beta)
%找到该段的汉明重量
k = 0;
wi = [];
operations = 2;%*******下述判断
...
if((I_i-I_iminus1)*(2*J_iminus1+(I_i-I_iminus1+1)*beta)/2>=Wi)%当前可靠度不能超出该区间最大值，在basic和1line中并未加此限制，是因为它们不可能达到该限制，因为最大可靠度是翻转所有比特
    ...
    operations = operations+6;%*******上述计算汉明重量下界
    operations = operations+5;%*******上述计算汉明重量上界
