...
for i = 1 : M
    ...
    for j = 1 : N
        if H(i, j) == 1
            H_row_one_absolute_index(i, cnt) = j;%记录原矩阵中每一行非零元素的列下标，行下标是对应的
            cnt = cnt + 1;
        end
    end
end
...
for i = 1 : M
    for j = 1 : cn_degree(i)
        H_column_one_relative_index(i, j) = 1 + sum(H(1 : i - 1, H_row_one_absolute_index(i, j)));%记录原矩阵中，每一行非零元素在列上的相对下标
    end
end
%VN degree polynomial
vn_distinct_degree_tmp = zeros(1, max(vn_degree));
for i = 1 : N
    vn_distinct_degree_tmp(vn_degree(i)) = vn_distinct_degree_tmp(vn_degree(i)) + 1;%遍历每个变量节点，得到每种度的个数，并存储在相应位置
end
