...
addpath('Tools');
addpath('My_LDPC/');
addpath('My_LDPC/my_5G_LDPC');
%% 获得H矩阵，并处理
k = 40;
R = 1/2;
[H, H_b, N, N_pun_inf, N_shor, N_pun_pc, Z_c] = Get_5GLDPC_H(k, R);
[H_column_permuted, M, N_decoding, K, vn_degree, cn_degree, P, H_row_one_absolute_index, H_column_one_relative_index, vn_distribution, cn_distribution] = H_matrix_process(H);
G = [eye(k+N_shor) P'];%生成矩阵
G(:,k+1:k+N_shor) = [];
G(end-N_shor+1:end,:) = [];
%% 找到最小的环，即围长
...
for i_vec = 1 : length(vec)
    ...
    for i_run = 1 : max_runs
        ...
        info_shor = [info zeros(1, N_shor)];%信息位缩短，补0，才能编码
        parity_check_bits = mod(info_shor*P', 2);%编码
        c = [info_shor parity_check_bits];%完整码字
        c_match = nrLDPC_rate_match(c, k, N_pun_pc, N_shor, N_pun_inf);%速率适配
        symbol = modulation(constellation, m, rho_inv, c_match', base_vec, N);%调制
        if is2D
        ...
        llr = demodulation(y, constellation, m, num_conste_points, demod_indices, sigma, N)';%LLR
        llr = nrLDPC_rate_recover(llr, k, N_pun_inf, N_shor, N_pun_pc);%恢复速率
       
        %BP,可统计操作数
        [operation_num, check_flag, c_hat, iter_this_time, output_modified_llr, output_modified_llr_pass_most_check] = OperationNum_modified_LDPC_Flooding_BP_decoder(llr, H_row_one_absolute_index, H_column_one_relative_index, N_decoding, M, vn_degree, cn_degree, max_iter);
...
%                 end
%             end
            %5G LDPC n=60 k=40 3db
            %temp = [0.3223 0.3360 0.3417];
            %5G LDPC n=80 k=40 3db
            temp = [0.3204  0.3368  0.3428 ];   
            %5G LDPC n=120 k=40 3db
            %temp = [0.3187  0.3385  0.3429 ];   
            %5G LDPC n=96 k=64 3db
            %temp = [0.3223   0.3360  0.3417  ];   
            %5G LDPC n=128 k=64 3db
            %temp = [ 0.4875   0.5125 ];  
            %5G LDPC n=192 k=64 3db
            %temp = [ 0.4849  0.5151 ]; 
            
            %获得修正后的LLR
            modified_llr = temp*[llr; output_modified_llr(1:alpha_OSD,:)];%累加

            %***********针对5G LPDC，去掉缩短位**********
            modified_llr(k+1:k+N_shor) = [];
            llr(k+1:k+N_shor) = [];
