...
while true
    ...
    while numel(buf) >= cfg.frameLen
        ...
        if coarseStart + cfg.frameLen - 1 > numel(buf)
            buf = buf(coarseStart:end);
            break;
        end

        oneFrame = buf(coarseStart : coarseStart + cfg.frameLen - 1);
        [bitsHat, eqSym, diagInfo] = decodeFrame(oneFrame, coarseCFO, cfg);
        ...
        if doPrint
            fprintf(['[RX] det=%5d | peak=%.3f | CFO=%8.1f Hz | longCorr=%.3f | ' ...
                     'pilotCorr=%.3f | SNR=%6.2f dB | BER=%8.3e | valid=%d\n'], ...
                stats.detectedFrames, peak, diagInfo.cfoHz, diagInfo.longCorr, ...
                diagInfo.pilotCorr, diagInfo.snrEst, ber, validated);
        end
...
searchRange = 0:(cfg.CP-1);   % 在CP内搜索最佳FFT窗口
