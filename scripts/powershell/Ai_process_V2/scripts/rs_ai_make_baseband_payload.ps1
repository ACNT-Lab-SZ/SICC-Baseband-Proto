...
$argsList = @()
$modeSuffix = switch ($ResourceMode) {
    'uniform' { '' }
    'importance' { '-importance' }
    'roi-layered' { '-roi-layered' }
}
switch ($Mode) {
    'video' {
        if (-not $VideoFile) {
        ...
        $argsList = @(
            '-m', 'rs_ai_link', ('encode-video' + $modeSuffix),
            '--video', $VideoFile,
            '--model', $ModelPath,
    ...
    'images' {
        ...
        $argsList = @(
            '-m', 'rs_ai_link', ('encode-images' + $modeSuffix),
            '--image-dir', $ImageDir,
            '--pattern', $Pattern,
            '--model', $ModelPath,
            '--output', $OutputBitstream,
    ...
    'viso' {
        ...
        $argsList = @('-m', 'rs_ai_link', ('encode-viso' + $modeSuffix))
        if ($SequenceRoot) {
...
if ($ResourceMode -eq 'importance') {
    $argsList += @(
        '--roi-conf', $RoiConf,
        '--roi-margin', $RoiMargin,
        '--bg-keep-ratio', $BgKeepRatio,
        '--bg-scale-boost', $BgScaleBoost
    )
} elseif ($ResourceMode -eq 'roi-layered') {
    $argsList += @(
        '--roi-conf', $RoiConf,
        '--roi-margin', $RoiMargin,
        '--bg-downsample', $BgDownsample
    )
}
