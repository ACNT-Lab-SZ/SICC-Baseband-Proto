...
EMERGENCY_MODEL="${EMERGENCY_MODEL:-models/emergency_xbd_damage_best.pt}"
TASK_ID="${TASK_ID:-disaster_damage_assessment}"
TASK_VERSION="${TASK_VERSION:-xbd_damage_v1}"
...
if [[ ! -f "$EMERGENCY_MODEL" ]]; then
  ...
  echo "[RS-AI] Train it first with scripts/rs_ai_train_xbd_damage_yolo.sh or set EMERGENCY_MODEL." >&2
  exit 2
fi
...
"${PYTHON_CMD[@]}" scripts/rs_ai_task_model_update.py make-update \
  ...
  --resource-scheme "$RESOURCE_SCHEME" \
  --output "$UPDATE_ZIP" \
  --notes "Satellite moved from VISO car monitoring to emergency disaster damage assessment."
...
"${PYTHON_CMD[@]}" scripts/rs_ai_task_model_update.py run-command \
  --sat-root "$SAT_ROOT" \
  --video "sample_data/xbd_emergency_demo/post_disaster.mp4" \
