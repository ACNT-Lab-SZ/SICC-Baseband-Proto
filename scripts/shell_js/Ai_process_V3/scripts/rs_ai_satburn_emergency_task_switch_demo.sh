...
EMERGENCY_MODEL="${EMERGENCY_MODEL:-models/emergency_satburn_burned_area_best.pt}"
TASK_ID="${TASK_ID:-wildfire_burned_area_assessment}"
TASK_VERSION="${TASK_VERSION:-satburn_burned_area_v1}"
SCENE="${SCENE:-wildfire_rescue}"
...
if [[ ! -f "$EMERGENCY_MODEL" ]]; then
  ...
  echo "[RS-AI] Train it first with scripts/rs_ai_train_satburn_yolo.sh or set EMERGENCY_MODEL." >&2
  exit 2
fi
...
"${PYTHON_CMD[@]}" scripts/rs_ai_task_model_update.py make-update \
  ...
  --resource-scheme "$RESOURCE_SCHEME" \
  --output "$UPDATE_ZIP" \
  --notes "Satellite task switched from VISO car monitoring to wildfire burned-area assessment."
...
"${PYTHON_CMD[@]}" scripts/rs_ai_task_model_update.py run-command \
  --sat-root "$SAT_ROOT" \
  --video "sample_data/satburn_demo/wildfire_burned_area_demo.mp4" \
