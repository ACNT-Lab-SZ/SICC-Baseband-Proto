...
EMERGENCY_MODEL="${EMERGENCY_MODEL:-models/emergency_quickquake_damage_best.pt}"
TASK_ID="${TASK_ID:-earthquake_building_damage}"
TASK_VERSION="${TASK_VERSION:-quickquake_damage_v1}"
SCENE="${SCENE:-earthquake_rescue}"
...
if [[ ! -f "$EMERGENCY_MODEL" ]]; then
  ...
  echo "[RS-AI] Train it first with scripts/rs_ai_train_quickquakebuildings_yolo.sh or set EMERGENCY_MODEL." >&2
  exit 2
fi
...
"${PYTHON_CMD[@]}" scripts/rs_ai_task_model_update.py make-update \
  ...
  --resource-scheme "$RESOURCE_SCHEME" \
  --output "$UPDATE_ZIP" \
  --notes "Satellite task switched from VISO car monitoring to earthquake building damage assessment."
...
"${PYTHON_CMD[@]}" scripts/rs_ai_task_model_update.py run-command \
  --sat-root "$SAT_ROOT" \
  --video "sample_data/quickquake_demo/earthquake_demo.mp4" \
