...
OUTPUT="${OUTPUT:-logs/baseband/rs_ai_tx_features_roi_layered.rsbf}"
INSPECT_JSON="${INSPECT_JSON:-logs/baseband/rs_ai_tx_features_roi_layered.inspect.json}"
MANIFEST_JSON="${MANIFEST_JSON:-logs/baseband/rs_ai_source_manifest_roi_layered.json}"
PATTERN="${PATTERN:-*.jpg}"
...
usage() {
  cat >&2 <<'USAGE'
usage:
  rs_ai_make_baseband_payload_roi_layered.sh --mode viso --video <video> --model <weights.pt> --output <tx.rsbf>

optional:
  --roi-conf 0.20 --roi-margin 0.10 --bg-downsample 4
USAGE
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --bg-downsample) BG_DOWNSAMPLE="$2"; shift 2 ;;
    -h|--help) usage; exit 0 ;;
...
case "$MODE" in
  video)
    ...
    "${PYTHON_CMD[@]}" -m rs_ai_link encode-video-roi-layered \
      --video "$VIDEO_FILE" \
  ...
  images)
    ...
    "${PYTHON_CMD[@]}" -m rs_ai_link encode-images-roi-layered \
      --image-dir "$IMAGE_DIR" \
      --pattern "$PATTERN" \
  ...
  viso)
    ...
    "${PYTHON_CMD[@]}" -m rs_ai_link encode-viso-roi-layered \
      "${SOURCE_ARGS[@]}" \
...
echo "[RS-AI] baseband_payload_roi_layered=$OUTPUT"
echo "[RS-AI] inspect_json=$INSPECT_JSON"
echo "[RS-AI] resource_mode=roi_layered_v1"
